<?php
	...
	function echoContest($info) {
        ...
        $ucontest = new UOJContest($info);
        $contest = $ucontest->info;

        $contest_name_link = '<a href="/contest/'.$contest['id'].'">'.$contest['name'].'</a>';
		...
		if ($contest['cur_progress'] == CONTEST_NOT_STARTED) {
			...
			if ($ucontest->userHasRegistered(Auth::user())) {
		...
		$click_zan_block = ClickZans::getBlock('C', $contest['id'], $contest['zan']);
		echo '<tr>';
...
<?php
	...
	$table_header .= '</tr>';
    echoLongTable(['*'], 'contests', [["status", "!=", 'finished']], 'order by start_time asc, id asc', $table_header,
		'echoContest',
		array('page_len' => 100)
	);
...
<?php
	echoLongTable(['*'], 'contests', ["status" => 'finished'], 'order by start_time desc, id desc', $table_header,
		'echoContest',
		array('page_len' => 100,
