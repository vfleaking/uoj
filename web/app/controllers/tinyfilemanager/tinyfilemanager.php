...
$CONFIG = '{"lang":"zh-CN","show_hidden":true,"hide_Cols":true,"calc_folder":false,"theme":"light"}';
...
//Application Title
defined('APP_TITLE') || define('APP_TITLE', 'Tiny File Manager');
...
$hide_Cols = defined('FM_DISABLE_COLS') || (isset($cfg->data['hide_Cols']) ? $cfg->data['hide_Cols'] : true);
...
$is_https = UOJContext::isUsingHttps();

// clean $root_url
$root_url = fm_clean_path($root_url);

// abs path for site
defined('FM_ROOT_URL') || define('FM_ROOT_URL', HTML::url($root_url));
defined('FM_SELF_URL') || define('FM_SELF_URL', HTML::url('?'));
...
if ($ip_ruleset != 'OFF') {
    ...
    if ($proceed == false) {
        ...
        if($ip_silent == false){
            UOJResponse::message('Access denied. IP restriction applicable', 'error');
        }
    ...
    switch (UOJRequest::post('type')) {
        ...
        case 'save':
            ...
            if ($write_results === false){
                UOJResponse::page500("Could Not Write File! - Check Permissions / Ownership");
            }
            die(true);
        ...
        // Save Config
        case 'settings':
            global $cfg, $lang, $show_hidden_files, $lang_list, $hide_Cols, $calc_folder, $theme;
            ...
            if (!defined('FM_DISABLE_COLS') && $cfg->data['hide_Cols'] != $hco) {
        ...
        //upload using url
        case 'upload':
            !empty($_REQUEST["uploadurl"]) || die('error');
            $path = FM_ROOT_PATH;
            if (FM_PATH != '') {
                $path .= '/' . FM_PATH;
            }

            $url = is_string($_REQUEST["uploadurl"]) && preg_match("|^http(s)?://.+$|", stripslashes($_REQUEST["uploadurl"])) ? stripslashes($_REQUEST["uploadurl"]) : null;
            ...
            if (preg_match("/^localhost$|^127(?:\.[0-9]+){0,2}\.[0-9]+$|^(?:0*\:)*?:?0*1$/i", $domain) || in_array($port, $knownPorts)) {
                ...
                die(json_encode(array("fail" => $err)));
            }
            ...
            if ($success) {
                @$success = stream_copy_to_stream($urlfp, $tmpfp, MAX_UPLOAD_SIZE);
                if (!$success) {
                    $err = error_get_last();
                }
                fclose($urlfp);
            ...
            if ($success) {
                fm_start_update();
                $success = rename($temp_file, $path."/".basename($fileinfo->name));
                if ($success) {
                    fm_end_update();
                    die(json_encode(array("done" => $fileinfo)));
                } else {
                    unlink($temp_file);
                    fm_end_update();
                }
...
if (isset($_POST['chmod']) && !FM_READONLY && !FM_IS_WIN && !$hide_Cols) {
    crsf_defend();
    fm_start_update();
    $path = FM_ROOT_PATH;
    if (FM_PATH != '') {
...
// update
if (isset($_POST['update']) && !FM_READONLY) {
    crsf_defend();
    if (isset($fm_hooks['update'])) {
        if ($fm_hooks['update']() !== false) {
            fm_set_msg('手动更新成功');
        } else {
            fm_set_msg('手动更新失败，呼叫管理员试试？', 'error');
        }
    }
    fm_redirect(FM_SELF_URL . '?p=' . urlencode(FM_PATH));
}
    ...
    <div class="path">
        ...
        <p>
            ...
            <b><a href="<?= HTML::url('?', ['params' => [
                ...
                'move' => 1,
                '_token' => crsf_token()
            ]]) ?>"><i class="fa fa-check-circle"></i> Move</a></b> &nbsp;
        ...
        <div class="card mb-2 <?php echo fm_get_theme(); ?>">
            ...
            <div class="card-body">
                <form id="js-settings-form" action="" method="post" data-type="ajax" onsubmit="return save_settings(this)">
                    ...
                    <?php if (!defined('FM_DISABLE_COLS')) { ?>
                    <div class="form-group row">
...
if (isset($_GET['view'])) {
    ...
    <div class="row">
        <div class="col-12">
            <?php if(!$quickView) { ?>
                <h3><?php echo $view_title ?> "<?php echo fm_enc(fm_convert_win($file)) ?>"</h3>
                <p class="break-word">
...
if (isset($_GET['chmod']) && !FM_READONLY && !FM_IS_WIN && !$hide_Cols) {
    $file = $_GET['chmod'];
    $file = fm_clean_path($file);
    $file = str_replace('/', '', $file);
    if ($file == '' || (!is_file($path . '/' . $file) && !is_dir($path . '/' . $file))) {
        ...
        <table class="table table-bordered table-hover table-sm <?php echo $tableTheme; ?>" id="main-table">
            ...
            foreach ($files as $f) {
                ...
                <tr>
                    ...
                    <td data-sort="b-<?= $modif_raw ?>"><?php echo $modif ?></td>
                    <?php if (!FM_IS_WIN && !$hide_Cols): ?>
    ...
    <div class="row">
        ...
        <div class="col-xs-12 col-sm-9">
            <ul class="list-inline footer-action">
                ...
                <li class="list-inline-item"><input type="submit" class="hidden" name="update" id="a-update" value="Update">
                    <a href="javascript:document.getElementById('a-update').click();" class="btn btn-small btn-outline-primary btn-2"><i class="fa fa-refresh"></i> <?php echo '手动更新' ?> </a></li>
            </ul>
        </div>
...
function fm_start_update() {
    global $fm_hooks;
    if (isset($fm_hooks['pre_update'])) {
        $fm_hooks['pre_update']();
    }
}

function fm_end_update() {
    global $fm_hooks;
    if (isset($fm_hooks['post_update'])) {
        $fm_hooks['post_update']();
    }
}
...
function fm_rdelete($path) {
    global $fm_hooks;
    $unlink = isset($fm_hooks['unlink']) ? $fm_hooks['unlink'] : 'unlink';
    $rmdir = isset($fm_hooks['rmdir']) ? $fm_hooks['rmdir'] : 'rmdir';
...
function fm_rename($old, $new) {
    ...
    $rename = isset($fm_hooks['rename']) ? $fm_hooks['rename'] : 'rename';

    return (!file_exists($new) && file_exists($old)) ? (bool)$rename($old, $new) : null;
}
...
function fm_copy($f1, $f2, $upd) {
    ...
    $copy = isset($fm_hooks['copy']) ? $fm_hooks['copy'] : 'copy';
    $ok = $copy($f1, $f2);
    if ($ok) {
        touch($f2, $time1);
    }
...
class FM_Zipper_Tar {
     ...
     */
    public function unzip($filename, $path) {
        $res = new PharData($filename);
        if ($res !== true) {
            return false;
        }
        if ($res->extractTo($path)) {
            return true;
        }
        return false;
    }
...
class FM_Config {
    ...
    function __construct() {
        ...
        $data = false;
        if (isset($_SESSION[FM_SESSION_ID]['CONFIG'])) {
            $data = fm_object_to_array(json_decode($_SESSION[FM_SESSION_ID]['CONFIG']));
    ...
    function save() {
        $_SESSION[FM_SESSION_ID]['CONFIG'] = json_encode($this->data);
    }
}
...
function fm_show_nav_path($path) {
    ...
    <nav class="navbar navbar-expand-lg <?php echo $getTheme; ?> mb-4 main-nav <?php echo $isStickyNavBar ?>">
        ...
        <div class="collapse navbar-collapse" id="navbarSupportedContent">
            ...
            <?php
            $path = fm_clean_path($path);
            $html = "<a href='?p='><i class='fa fa-home' aria-hidden='true' title='" . FM_ROOT_PATH . "'></i></a>";
            ...
            if ($path != '') {
                ...
                $html .= $sep . implode($sep, $array);
            }
            echo '<div class="col-xs-6 col-sm-5">' . $html . '</div>';
            ?>
...
<head>
    ...
    <!-- Bootstrap core CSS -->
    <?= HTML::css_link('/libs/bootstrap-4.5.3/css/bootstrap.min.css') ?>
    <?= HTML::css_link('/libs/font-awesome-4.7.0/css/font-awesome.min.css') ?>
    <?= HTML::css_link('/libs/lightbox-5.3.0/ekko-lightbox.css') ?>
    <?= HTML::css_link('/libs/datatables-bs4-dt-1.10.25/datatables.css') ?>
 
    <?php if (FM_USE_HIGHLIGHTJS): ?>
        <?= HTML::css_link('/libs/highlight-11.0.1/styles/'.FM_HIGHLIGHTJS_STYLE.'.min.css') ?>
    <?php endif; ?>
    <style>
...
<?= HTML::js_src('/libs/jquery-3.6.0.min.js') ?>
<?= HTML::js_src('/libs/bootstrap-4.5.3/js/bootstrap.min.js') ?>
<?= HTML::js_src('/libs/datatables-bs4-dt-1.10.25/datatables.min.js') ?>
<?= HTML::js_src('/libs/lightbox-5.3.0/ekko-lightbox.min.js') ?>
<?php if (FM_USE_HIGHLIGHTJS): ?>
    <?= HTML::js_src('/libs/highlight-11.0.1/highlight.min.js') ?>
...
<script>
    ...
    function newfolder(e) {
        ...
        null !== t && "" !== t && n && (window.location.hash = "#", window.location.search = "p=" + encodeURIComponent(e)
            + "&new=" + encodeURIComponent(t)
            + "&type=" + encodeURIComponent(n)
            + "&_token=" + <?= json_encode(crsf_token()) ?>
        )
    }
    function rename(e, t) {
        var n = prompt("New name", t);
        null !== n && "" !== n && n != t && (window.location.search = "p=" + encodeURIComponent(e)
            + "&ren=" + encodeURIComponent(t)
    ...
    function backup(e, t) { //Create file backup with .bck
        var n = new XMLHttpRequest,
            a = "path=" + e + "&file=" + t + "&type=backup&ajax=true" + "&_token=" + <?= json_encode(crsf_token()) ?>;
...
<?php if (isset($_GET['edit']) && isset($_GET['env']) && FM_EDIT_FILE):
        ...
        ?>
    <?= HTML::js_src('/libs/ace-1.4.12/ace.js') ?>
    <?= HTML::js_src('/libs/ace-1.4.12/ext-modelist.js') ?>
    <script>
        var editor = ace.edit("editor");
        var modelist = ace.require("ace/ext/modelist");
        var mode = modelist.getModeForPath(<?= json_encode($_GET['edit']) ?>).mode
        ...
        function renderThemeMode() {
            var $modeEl = $("select#js-ace-mode"),
                ...
                optionNode = function(type, arr){
                    var $Option = ""; $.each(arr, function(i, val) {
            ...
            $(".js-ace-toolbar").on("click", 'button', function(e){
                ...
                if(cmdValue && cmdValue != "none") {
                    ...
                    if(editorOption == "fullscreen") {
                        ...
                        var helpHtml="";$.each(aceHelp,function(i,value){helpHtml+="<li>"+value+"</li>";});var tplObj={id:1028,title:"Help",action:false,content:helpHtml},tpl=$("#js-tpl-modal").html();$('#wrapper').append(template(tpl,tplObj));$("#js-ModalCenter-1028").modal('show');
                    }
                }
            });
...
function lng($txt) {
    ...
    $tr['en']['Advanced Search']    = 'Advanced Search';    $tr['en']['Error while copying fron']    = 'Error while copying from';
