...
function svnSyncProblemData($problem, $user = null) {
	return (new SvnProblemDataManager($problem, $user))->sync();
}
function svnAddHackPoint($problem, $uploaded_input_file, $uploaded_output_file, $reason = null, $user = null) {
	if ($reason === null) {
		if (UOJHack::cur()) {
			$reason = [
	...
	return (new SvnProblemDataManager($problem, $user))->addHackPoint($uploaded_input_file, $uploaded_output_file, $reason);
}

function svnUploadViaBrowser($problem, $new_data_zip) {
	return (new SvnProblemDataManager($problem))->upload($new_data_zip);
}

function svnUpdateProblemConf($problem, $new_problem_conf) {
	return (new SvnProblemDataManager($problem))->updateProblemConf($new_problem_conf);
}
